# StuffTracker — Implementationsplan v2
...
**Mål:** Kunna skapa Room, Storage och Position under ett Home. Validera att hierarkin följer reglerna.
...
- [ ] Skapa validator med regler:
  ...
  - LocationType: måste vara Room, Storage eller Position (inte Home eller Unsorted)
...
- [ ] Hämta parent-location från repository
- [ ] Validera tillåtna parent-child-kombinationer:
  - Room → måste ha Home som parent
  - Storage → måste ha Room eller Storage som parent
  - Position → måste ha Storage som parent
- [ ] Kasta lämpligt exception vid ogiltiga kombinationer (skapa t.ex. `BusinessRuleException` i Domain)
- [ ] Skapa `LocationDto` (Id, Name, Description, LocationType, ParentId, CreatedAt)
...
- [ ] Handlern ska kopiera `HomeId` från parent-location automatiskt
- [ ] Testa hela kedjan i Scalar:
  ...
  4. Skapa en position under förvaringsplatsen (LocationType=Position)
  5. Försök skapa ett rum under en förvaringsplats → bör ge 400/422
...
- [ ] Exempelfråga:
  ...
  ```
- [ ] Verifiera att den returnerar korrekt kedja: Home > Room > Storage > Position
...
| 1 | Skapa platser | Hierarkivalidering, parent-child-regler |
...
### Guid-generering
Generera Guid i handler eller command (inte i repository/EF). Det ger dig kontroll och gör det möjligt att returnera Id direkt utan extra roundtrip. Den nuvarande koden genererar Guid i AutoMapper-profilen — flytta detta till handlern.
...
### Testning
Planen fokuserar på manuell testning via Scalar. Vill du lägga till automatiserade tester (unit tests för handlers, integration tests för API) är det ett naturligt tillägg efter att grundfunktionaliteten fungerar. Överväg att lägga till ett testprojekt vid ett lämpligt tillfälle — det stärker ett portfolio-projekt.
